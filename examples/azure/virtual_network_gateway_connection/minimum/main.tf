...
module "root" {
  ...
  azure_virtual_network_gateway_connections = {
    minimum = {
      ...
      connection_name             = "conn-test-er"
      location                    = "ref:azure_resource_groups.test.location"
      connection_type             = "ExpressRoute"
      virtual_network_gateway_id  = "ref:azure_virtual_network_gateways.test.id"
      express_route_circuit_id    = "ref:azure_express_route_circuits.test.id"
      virtual_network_gateway1_id = "ref:azure_virtual_network_gateways.test.id"
    }
  }
}
