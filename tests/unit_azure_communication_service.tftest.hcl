# Unit test — modules/azure/communication_service
# Tests the sub-module in isolation (plan only).
# Run: terraform test -filter=tests/unit_azure_communication_service.tftest.hcl
...
run "plan_communication_service" {
  ...
  assert {
    condition     = output.id == "/subscriptions/${var.subscription_id}/resourceGroups/rg-test/providers/Microsoft.Communication/communicationServices/test-acs-main"
    error_message = "ARM ID must be correctly formed."
  }

  assert {
    condition     = output.name == "test-acs-main"
